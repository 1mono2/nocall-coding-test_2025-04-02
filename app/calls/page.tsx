...
import type { CallDTO } from "@/app/dto/types";
...
export default function CallsPage() {
	...
	// APIクライアントの初期化
	const client = useMemo(() => hc<AppType>("/").api.v1, []);
	...
	useEffect(() => {
		fetchCalls();
		fetchCustomers();
	}, [fetchCalls, fetchCustomers]);

	// コールステータスを更新する関数
	const updateCallStatus = useCallback(
		async (callId: string, action: "start" | "complete" | "cancel") => {
			if (!client) return;
			...
			try {
				...
				if (action === "start") {
					response = await client.calls[":id"].start.$post({
						param: { id: callId },
					});
				} else if (action === "complete") {
					response = await client.calls[":id"].complete.$post({
						param: { id: callId },
					});
				} else if (action === "cancel") {
					response = await client.calls[":id"].cancel.$post({
						param: { id: callId },
					});
				}
	...
	// ステータスに応じた操作ボタンを表示
	const renderActionButtons = (call: CallDTO) => {
		switch (call.status) {
			case "queued":
				return (
		...
		<div className="space-y-4">
			<div>
				...
				<p className="text-muted-foreground">
					登録されているすべてのコールを表示します
				</p>
			</div>

			{loading ? (
				<div className="flex justify-center py-8 text-foreground">
					読み込み中...
				</div>
			) : calls.length === 0 ? (
